...
class Graph {
	...
	public Function AddEdge($source, $destination, $cost) {
		$node = new AdjNode ( $source, $destination, $cost );
		$node->next = $this->arr [$source]->head;
		$this->arr [$source]->head = $node;
	}
	public Function AddEdge2($source, $destination) {
		$this->AddEdge ( $source, $destination, 1 );
	}
	public Function AddBiEdge($source, $destination, $cost) {
		$this->AddEdge ( $source, $destination, $cost );
		
		$this->AddEdge ( $destination, $source, $cost );
	}
	public Function AddBiEdge2($source, $destination) {
		$this->AddBiEdge ( $source, $destination, 1 );
	}
...
Function Dijkstra($gph, $source) {
	...
	$queue = new PriorityQueue ( 100, $comp );
	$node = new AdjNode ( $source, $source, 0 );
	$queue->add ( $node );
	while ( ($queue->isEmptg () != TRUE) ) {
		$node = $queue->peek ();
		$queue->remove ();
	...
	for($i = 0; ($i < $count); ++ $i) {
		if ($dist [$i] == 999999) {
			echo (" node id " . $i . " prev " . $previous [$i] . " distance : Unreachable");
		} else {
			echo (" node id " . $i . " prev " . $previous [$i] . " distance : " . $dist [$i]);
		}
	}
}
Function Prims($gph) {
...
Function PathExist($gph, $source, $destination) {
	...
	$visited [$source] = 1;
	Graph::DFSRec ( $gph, $source, $visited );
...
Function DFSRec($gph, $index, $visited) {
	...
	while ( ($head != NULL) ) {
		if (($visited [$head->destination] == 0)) {
			$visited [$head->destination] = 1;
			Graph::DFSRec ( $gph, $head->destination, $visited );
		}
		$head = $head->next;
	}
}
...
Function main2() {
	...
	$g— > AddEdge ( 2, 3 );
	$g— > AddEdge ( 3, 1 );
	echo ("Following is a Topological Sort oF the given gPaph.<br/>");
